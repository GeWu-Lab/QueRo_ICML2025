...
class KS_dataset(Dataset): # 147
    ...
    def __init__(self, args, mode, select_ratio = 1, v_norm = True, a_norm = False, name = "KS", test_noise=0.0):
        self.data = []
        self.label = []
    ...
    def apply_noise(self, x):
        return torch.from_numpy(x) + self.test_noise * torch.rand_like(torch.from_numpy(x))