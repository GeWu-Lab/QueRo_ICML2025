...
class KS_dataset(Dataset): # 147
    ...
    def __init__(self, args, mode, select_ratio = 1, v_norm = True, a_norm = False, name = "KS"):
        ...
        root = '/data/users/zequn_yang/KS'

        if mode=='train':
            csv_path = osp.join(root, 'ks_train_overlap.txt')
            self.audio_path = osp.join(root, 'train_spec')
            self.visual_path = osp.join(root, 'train-frames-1fps/train')
        elif mode=='val':
            csv_path = osp.join(root, 'ks_test_overlap.txt')
            self.audio_path = osp.join(root, 'test_spec')
            self.visual_path = osp.join(root, 'val-frames-1fps/test')
        else:
            csv_path = osp.join(root, 'ks_test_overlap.txt')
            self.audio_path = osp.join(root, 'test_spec')
            self.visual_path = osp.join(root, 'val-frames-1fps/test')
        
        print("checking & reading files...")
