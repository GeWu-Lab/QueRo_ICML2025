...
def set_affine_matrix(args, model, AIR, device, dataloader):
    model.eval()
    ...
    with torch.no_grad():
        ...
        print('Calculating affine matrix')
        affine_matrix = compute_rotation_matrix(X=v, Y=target_cls)
        print('Done.')

        model.module.fusion_module.set_affine(affine_matrix.float(), target_cls)
