...
def train(args, epoch, model, device, dataloader, optimizer, scheduler, writer=None, logger=None):
    ...
    if args.fusion_method == 'AttnFusion':
...
def train_attn(args, epoch, model, criterion, device, dataloader, optimizer, scheduler, writer=None, logger=None):
    ...
    for step, (data) in tqdm(enumerate(dataloader)):
        ...
        print("seq len a,v", seq_a_len, seq_v_len, attn_matrix.shape)
    ...
    with torch.no_grad():
        ...
        if args.modulation == "affine" and model.module.fusion_module.r_time < args.r_time:
            if torch.abs(AIR) > args.beta:
                print(f"Set Affine Matrix at {epoch}")
                set_affine_matrix(args=args, model=model, AIR=AIR, device=device, dataloader=dataloader)
                with open(os.path.join(args.exp_root, 'AIR.txt'), '+a') as file:
                    file.write(f"Set Affine Matrix at {epoch}\n")
